    ...
    <main className="container mx-auto px-4 sm:px-6 lg:px-8 mt-8 sm:mt-12">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16">
        ...
        <div>
          ...
          <p className="text-xl sm:text-2xl mt-2 text-brand-text">{formatCurrency(product.price)}</p>
          <p className="mt-4 text-sm text-stone-600 leading-relaxed">{t(product.description)}</p>
          
          {/* Color Selector */}
          <div className="mt-6">
            <p className="text-sm font-medium">{t('product_color')}: <span className="text-stone-600">{t(selectedColor)}</span></p>
            <div className="flex space-x-2 mt-2">
              {product.colors.map(color => (
                <button 
                  key={color.name}
                  onClick={() => setSelectedColor(color.name)}
                  className={`w-8 h-8 rounded-full border-2 ${selectedColor === color.name ? 'border-brand-brown' : 'border-transparent'}`}
                  style={{ backgroundColor: color.hex, outlineOffset: '2px' }}
                  title={t(color.name)}
      ...
      <section className="mt-16 sm:mt-24">
        <h2 className="text-2xl font-medium text-center mb-8 sm:mb-10">{t('product_related_title')}</h2>
